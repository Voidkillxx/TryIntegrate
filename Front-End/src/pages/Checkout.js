...
import { checkout, fetchProfile } from '../utils/api'; // Import fetchProfile
import '../Styles/Checkout.css';

const Checkout = ({ showAlert }) => {
  const { cartItems, selectedItems, selectedSubtotal, refreshCart } = useContext(CartContext);
  const navigate = useNavigate();
  
  const [validated, setValidated] = useState(false);
  const [showConfirmOrderModal, setShowConfirmOrderModal] = useState(false);
  
  // Form State - Pre-fill logic handled in useEffect
      ...
      try {
        ...
        if (user) {
            ...
            setShippingAddress({
                ...
                city: 'Cabuyao City', // Default based on user's address location
                zip: user.zipcode || '',
  ...
  const handleConfirmOrderPlacement = async () => {
    setLoading(true);
    try {
        // FIX: Construct the shipping_address string using ONLY address, city, and zip.
        // We exclude name and phone from the physical address field.
        const coreAddress = `${shippingAddress.address}, ${shippingAddress.city}, ${shippingAddress.zip}`;
        
        const payload = {
            // Send only the necessary address components to the backend
        ...
        // FIX: The backend is now guaranteed to work (with stock decrement), so just refresh context
        await refreshCart(); 
